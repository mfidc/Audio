...
class VAD:
    def __init__(self, model_path, silence_threshold=2.0):
    ...
    def vad_states(self, vad_confidence):
        if vad_confidence > 0.5:  # Slightly higher threshold to start speaking
            ...
            if self.current_state == "Speaking":
                if self.last_speech_time and (time.time() - self.last_speech_time > self.silence_threshold):
                    self.current_state = "Silence"
                else:
...
class RollingBuffer:
    def __init__(self, duration, sample_rate, chunk_size):
        self.buffer_size = int(duration * sample_rate / chunk_size)
...
def test_vad(audio_input, noise_reducer, vad, pre_buffer_duration=0.5):
    ...
    rolling_buffer = RollingBuffer(pre_buffer_duration, audio_input.sample_rate, audio_input.chunk_size)
    is_new_speech = False
    try:
        while True:
            audio_chunk = audio_input.record()
            rolling_buffer.add(audio_chunk)
            ...
            if vad_state == "Speaking":
                if not is_new_speech:
                    ...
                    buffer.extend(rolling_buffer.get_buffer())
                    is_new_speech = True
                buffer.append(audio_chunk)
            else:
                is_new_speech = False  # Reset flag on non-speech
